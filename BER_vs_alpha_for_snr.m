...
Nc= 2; % No. of Rx Antennas at Charlie
...
Charlie_p10= zeros(Nc, L);
Charlie_p01 = zeros(Nc, L);
...
Bob_BER(s,k)=compute_BER(total_bits,Bob_err);
...
hold on;
%C={'mo-','r*-','b+-','yx-'};
colororder(lines(length(SNR_dB)));
for p=1:length(SNR_dB)

semilogy(alpha, Bob_BER(p,:), 'LineWidth', 1, 'MarkerSize', 2);
end

hold off;
xlabel('\alpha'); ylabel('Prob of error ');

title(sprintf("Bob BER versus alpha for Nc = %d",Nc));
legend('SNR=20dB', 'SNR=25dB','SNR=30dB', 'SNR=35dB');
grid on;

filename="RESULTS_SNR 20_25_30_35__Nc_2_ITR_1000.mat";
save(filename)
